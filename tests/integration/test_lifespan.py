...
async def test_application_starts_and_stops_cleanly() -> None:
...
async def test_redis_is_connected_during_startup_and_the_pool_is_released_after() -> None:
    app = create_app(_settings())
...
async def test_database_engine_is_usable_during_startup() -> None:
...
async def test_startup_registers_the_secret_for_redaction() -> None:
...
async def test_starting_twice_in_one_process_is_safe() -> None:
