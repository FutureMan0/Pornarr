          ...
          {requests.data?.map((request) => (
            <li key={request.id} className="border border-border p-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h2 className="font-medium text-ink">{request.query}</h2>
                  <p className="text-sm text-ink-muted">
                    {t(`requests.status.${request.status}`)} ·{" "}
                    {t("requests.priority", { value: request.priority })}
                  </p>
                </div>
                <div className="flex gap-2">
                  {request.status === "failed" || request.status === "not_found" ? (
                    <button
                      type="button"
                      className="button"
                      onClick={() => action.mutate({ id: request.id, action: "retry" })}
                    >
                      {t("requests.retry")}
                    </button>
                  ) : null}
                  {!TERMINAL.has(request.status) ? (
                    <button
                      type="button"
                      className="button"
                      onClick={() => action.mutate({ id: request.id, action: "cancel" })}
                    >
                      {t("requests.cancel")}
                    </button>
                  ) : null}
              ...
              <ol
                className="mt-3 flex flex-wrap gap-2 text-sm text-ink-muted"
                aria-label={t("requests.history")}
              >
                {request.history.map((item, index) => (
                  <li key={`${item.status}-${index}`}>{t(`requests.status.${item.status}`)}</li>
                ))}
              </ol>
            </li>
          ))}
